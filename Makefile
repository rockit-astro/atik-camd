...
SERVER_VERSION=$(shell awk '/Version:/ { print $$2; }' atik-camera-server.spec)

all:
	mkdir -p build
	cp camd camd.bak
	awk '{sub("SOFTWARE_VERSION = .*$$","SOFTWARE_VERSION = \"$(SERVER_VERSION) ($(GIT_VERSION))\""); print $0}' camd.bak > camd
	${RPMBUILD} -ba atik-camera-server.spec
	${RPMBUILD} -ba atik-camera-client.spec
	${RPMBUILD} -ba python3-warwick-atik-camera.spec
	mv build/noarch/*.rpm .
	rm -rf build
	mv camd.bak camd
